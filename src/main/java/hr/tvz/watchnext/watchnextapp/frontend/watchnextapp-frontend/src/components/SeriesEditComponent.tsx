    ...
    <div style={editContainerStyle}>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
                <label style={{ ...labelStyle, flex: 1 }}>Broj sezona
                    <input 
                        type="number"
            ...
            <div style={{ display: 'flex', gap: '12px' }}>
                <label style={{ ...labelStyle, flex: 1 }}>IMDB Rating
                    <input
                        type="number"
                        step="0.1"
                        min="0"
                        ...
                        onChange={e => setFormData({...formData, imdbRating: Number(e.target.value)})}
                        style={inputStyle}
                    />
                </label>

                <label style={{ ...labelStyle, flex: 1 }}>IMDB ID
                    <input
                        value={formData.imdbId ?? ''}
                        onChange={e => setFormData({...formData, imdbId: e.target.value})}
                        style={inputStyle} placeholder="npr. tt0903747"
                    />
                </label>
            </div>
